...
@section('content')
    @php
        $brandColor = $headerColor ?? ($newsletterSettings['insight_brand_color'] ?? '#0d1b2a');
        $ctaText = filled($ctaText ?? null) ? trim((string) $ctaText) : 'Read more on Dataphyte';
        $ctaUrl = filled($ctaUrl ?? null) ? trim((string) $ctaUrl) : 'https://www.dataphyte.com';
        $insightContent = preg_replace(
            '/^\s*<(p|h[1-6])\b[^>]*>\s*(?:<(strong|b)\b[^>]*>\s*)?Subject\s*:\s*.*?<\/\1>\s*/is',
            '',
            $content ?? ''
        );
        $insightContent = preg_replace(
