@php
    $socialLinks = collect($footerConfig['social_links'] ?? [])
        ->filter(fn ($url) => filled($url))
        ->all();
    $offices = $footerConfig['offices'] ?? [];
    $websiteUrl = $footerConfig['website_url'] ?? 'https://dataphyte.org';
@endphp
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
            ...
            <tr>
                <td style="padding:18px 0 0;text-align:center;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:11px;line-height:1.6;color:#8a8f98;">
                    &copy; {{ date('Y') }}
                    <a href="{{ $websiteUrl }}" target="_blank" rel="noopener noreferrer" style="color:#8a8f98;text-decoration:underline;">{{ $fromName ?? config('app.name') }}</a>.
                    All rights reserved.
                </td>
            </tr>
        </table>
